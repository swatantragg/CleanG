...
async def upload_source(branch_id: int, file: UploadFile, user: User = CurrentUser, db: AsyncSession = Depends(get_db)):
    """Source files are stored as TEXT in the DB — never sent to object storage."""
    ...
    row = File(
        branch_id=branch.id, kind="source", storage_key="", content=text,
...
async def stream(file_id: int, token: str, db: AsyncSession = Depends(get_db)):
    ...
    if f.storage_key:
        ...
        # source file → text stored in the DB
